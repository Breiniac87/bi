    ...
    if df_filtered_base.empty:
        ...
        with col_p1:
            selected_player = st.selectbox(
                ...
                key="player_impact_1"  # ОБНОВЛЕННЫЙ КЛЮЧ
            )
        
        with col_p2:
            default_idx_1 = plot_metrics.index("Цена после СПП") if "Цена после СПП" in plot_metrics else 0
            metric_1 = st.selectbox("Метрика 1 (левая ось Y):", plot_metrics, index=default_idx_1, key="metric1_impact_1")  # ОБНОВЛЕННЫЙ КЛЮЧ
            
        with col_p3:
            default_idx_2 = plot_metrics.index("Заказов шт. (всего)") if "Заказов шт. (всего)" in plot_metrics else min(1, len(plot_metrics)-1)
            metric_2 = st.selectbox("Метрика 2 (правая ось Y):", plot_metrics, index=default_idx_2, key="metric2_impact_1")  # ОБНОВЛЕННЫЙ КЛЮЧ
    ...
    if df_filtered_base.empty:
        ...
        with col_filters_2:
            ...
            selected_metric_2 = st.selectbox(
                ...
                key="metric_comp_2"  # ОБНОВЛЕННЫЙ КЛЮЧ
            )
            selected_sellers_2 = st.multiselect(
                "Выберите продавцов для сравнения:", 
                all_sellers, 
                default=default_sellers, 
                key="sellers_comp_2"  # ОБНОВЛЕННЫЙ КЛЮЧ
            )
    ...
    if df_filtered_base.empty:
        ...
        with col_p1_2:
            selected_player_2 = st.selectbox(
                ...
                all_sellers, 
                index=0 if all_sellers else None,
                key="player_impact_2"  # ОБНОВЛЕННЫЙ КЛЮЧ
            )
        
        with col_p2_2:
            default_idx_1_2 = plot_metrics.index("Цена после СПП") if "Цена после СПП" in plot_metrics else 0
            metric_1_2 = st.selectbox("Метрика 1 (левая ось Y):", plot_metrics, index=default_idx_1_2, key="metric1_impact_2")  # ОБНОВЛЕННЫЙ КЛЮЧ
            
        with col_p3_2:
            default_idx_2_2 = plot_metrics.index("Заказов шт. (всего)") if "Заказов шт. (всего)" in plot_metrics else min(1, len(plot_metrics)-1)
            metric_2_2 = st.selectbox("Метрика 2 (правая ось Y):", plot_metrics, index=default_idx_2_2, key="metric2_impact_2")  # ОБНОВЛЕННЫЙ КЛЮЧ
